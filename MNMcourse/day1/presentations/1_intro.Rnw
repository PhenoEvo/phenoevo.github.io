...
<<label=Rcode, include=FALSE, cache=TRUE>>=
library(deSolve)
library(ecostudy)

data(lynxhare)

lynxhare_df <- as.data.frame(lynxhare)
...
out_df$x_scaled <- as.numeric(out_df$x) *
  max(lynxhare_df$Hare) / max(out_df$x)

out_df$y_scaled <- as.numeric(out_df$y) *
  max(lynxhare_df$Lynx) / max(out_df$y)
@
\frame{\titlepage}
    ...
    \begin{overlayarea}{\textwidth}{0.7\textheight}
      \only<1>{
        \begin{figure}
          \begin{tikzpicture}
            \node (g_picture) {
              \includegraphics[width=0.4\textwidth]{../../resources/images/grinnell.png}
            };
            \node[anchor=north] at (g_picture.south) (g_book) {
              \includegraphics[width=\textwidth]{../../resources/images/grinnell_book.png}
            };
          \end{tikzpicture}
        \end{figure}
      }
      \only<2>{
        \begin{figure}
          \begin{tikzpicture}
            \node (e_picture) {
              \includegraphics[width=0.4\textwidth]{../../resources/images/elton.jpg}
            };
          \end{tikzpicture}
        \end{figure}
      }
...
\begin{frame}
  \frametitle{Mechanistic Ecological Niche Models}
  ...
  \begin{figure}
    \begin{columns}[c]
      \begin{column}{0.45\textwidth}
        \centering
        \only<1->{
          \includegraphics[height=0.6\textheight]{../../resources/images/hypervolume.png}
        }
      \end{column}
      \begin{column}{0.45\textwidth}
        \centering
        \only<2>{
          \includegraphics[height=0.6\textheight]{../../resources/images/hypervolume_in_space1.jpg}
        }
        \only<3>{
          \includegraphics[height=0.6\textheight]{../../resources/images/hypervolume_in_space2.jpg}
        }
      \end{column}
    \end{columns}
...
\subsection{What are mechanistic models}
...
\begin{frame}
  ...
  \begin{figure}
    \centering
    {\centering
      \includegraphics[width=0.25\textwidth]{../../resources/images/rabbit.jpg}
      \hspace{0.5cm}
      \includegraphics[width=0.23\textwidth]{../../resources/images/lynx.jpg}
      \par}

    \vspace{0.2cm}
    \includegraphics[width=0.6\textwidth]{../../resources/images/lv-model.png}
    \caption{\tiny Population dynamics of rabbits and lynx}
    \label{fig:lv-model}
  \end{figure}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{0.55\textwidth}
      \footnotesize
      \textbf{Parameters:}\\
      \textbf{\(x\)}: density of prey (e.g., rabbits) \\
      \textbf{\(y\)}: density of predators (e.g., foxes) \\
...
par(mfrow = c(2, 1), mar = c(4.5, 4, 0, 0))

# TIME SERIES

plot(lynxhare_df$Year, lynxhare_df$Hare, type = "n", xlab = "Year", ylab = "Population", ylim = c(0, 160), las = 1, )
grid(col = "grey90")
lines(lynxhare_df$Year, lynxhare_df$Hare, col = hare_col, lwd = 2.5, type = "b", pch = 16)
lines(lynxhare_df$Year, lynxhare_df$Lynx, col = lynx_col, lwd = 2.5, type = "b", pch = 16)
lines(lynxhare_df$Year, out_df$x_scaled, col = hare_col, lwd = 2, lty = 2)
lines(lynxhare_df$Year, out_df$y_scaled, col = lynx_col, lwd = 2, lty = 2)
legend("topright",
  legend = c("Observed Hare", "Observed Lynx", "Model Hare", "Model Lynx"),
  col = c(hare_col, lynx_col, hare_col, lynx_col), lty = c(1, 1, 2, 2),
  pch = c(16, 16, NA, NA), lwd = c(2.5, 2.5, 2, 2), cex = 0.6, bty = "n"
)

# PHASE PORTRAIT

plot(lynxhare_df$Hare, lynxhare_df$Lynx,
  pch = 16, col = "black",
  xlab = "Hare", ylab = "Lynx", xlim = c(0, 160), ylim = c(0, 80), las = 1
)
grid(col = "grey90")
lines(out_df$x_scaled, out_df$y_scaled, col = 2, lwd = 2.5)
legend("topright",
  legend = c("Observed data", "LV model"),
  pch = c(16, NA), lty = c(NA, 1), col = c(1, 2), lwd = c(NA, 2), cex = 0.7, bty = "n"
)
@
...
\begin{frame}
  ...
  \begin{figure}
    \includegraphics[height=0.8\textheight]{../../resources/images/models.png}
    \caption{\tiny models and theory... Taken from \textcite{kearney2021a}}
...
\begin{frame}
  ...
  \only<1>{
    \begin{figure}
      \includegraphics[height=0.8\textheight]{../../resources/images/sdm_mods_1.png}
      \newline \tiny
      \caption{\tiny caption}
  ...
  \only<2>{
    \begin{figure}
      \includegraphics[height=0.8\textheight]{../../resources/images/sdm_mods_2.png}
      \newline \tiny
      \caption{\tiny caption}
      \label{fig:sdm-models}
    \end{figure}
  ...
  \only<3>{
    \begin{figure}
      \includegraphics[height=0.8\textheight]{../../resources/images/sdm_mods_3.png}
      \newline \tiny
      \caption{\tiny caption}
      \label{fig:sdm-models}
    \end{figure}
  ...
  \end{textblock*}
  \begin{textblock*}{2cm}(8.6cm, 7.5cm)
...
\begin{frame}
  ...
  \only<1>{
    \begin{figure}
      \includegraphics[height=0.6\textheight, trim={4cm 2cm 4cm 4cm}, clip]{../../resources/images/extrapolation1.png}
      \caption{\tiny Taken from \textcite{maino2016}}
  ...
  \only<2>{
    \begin{figure}
      \includegraphics[height=0.6\textheight, trim={4cm 2cm 4cm 4cm}, clip]{../../resources/images/extrapolation2.png}
      \caption{\tiny Taken from \textcite{maino2016}}
      \label{fig:extrapolation}
    \end{figure}
  }
  \only<3>{
    \begin{figure}
      \includegraphics[height=0.6\textheight, trim={4cm 2cm 4cm 4cm}, clip]{../../resources/images/extrapolation3.png}
      \caption{\tiny Taken from \textcite{maino2016}}
      \label{fig:extrapolation}
    \end{figure}
  }
...
\begin{frame}
  ...
  \begin{figure}
    \begin{columns}[c]
      ...
      \begin{column}{0.45\textwidth}
        \centering
        \includegraphics[width=0.46\textwidth]{../../resources/images/gates.jpg}
        \hspace{0.4cm}
        \includegraphics[width=0.25\textwidth]{../../resources/images/porter.jpg}
        \par
        \vspace{0.5cm}
        \includegraphics[height=0.5\textheight]{../../resources/images/gates_book.jpg}
      \end{column}
      \begin{column}{0.45\textwidth}
        \visible<2->{
          \centering
          \includegraphics[width=0.30\textwidth]{../../resources/images/bas.jpg}
          \par
          \vspace{0.5cm}
          \includegraphics[height=0.5\textheight]{../../resources/images/bas_book.jpg}
        }
      \end{column}
    \end{columns}
  \end{figure}
