...
title: |
        Missing Data
        <br><img src="LectureData/08.missing/LizardMissing.png" style="width:500px; margin: 20px auto 20px auto;"><br>
...
* GPA: 
  * Aligns specimens to the average shape (reference, consensus shape)
...
include_graphics("lectureData/08.missing/MissingAnthro.png")
```
...
include_graphics(c("lectureData/08.missing/FishMissingStart.png",
                   "lectureData/08.missing/arrow.png", 
                   "lectureData/08.missing/FishMissingDelSpec.png"))
```
...
include_graphics("lectureData/08.missing/FishMissingStart.png")
```
...
include_graphics("lectureData/08.missing/Skull1.png")
```
...
```{r echo=FALSE, eval=TRUE,  out.width="40%", fig.align='center'}
mydat <- readland.tps("lectureData/08.missing/LizardShape.tps", warnmsg = FALSE)
...
links <- read.table("lectureData/08.missing/lizardlinks.txt",header=T)
ref <- mshape(shape)
...
```{r, echo = FALSE, out.width="30%", fig.align='center'}
include_graphics("lectureData/08.missing/LizardMissing.png")
```
...
mtext("Original Specimen", line = -7, font = 2)
plotRefToTarget(shape[,,62],spec,links=links)
mtext("TPS: Original --> Estimated Specimen",  line = -7, font = 2)
par(mfrow=c(1,1)) 
```
...
spec <- readland.tps('lectureData/08.missing/fish.tps', specID = "imageID", warnmsg = FALSE)
spec <- geomorph:::center.scale(spec[,,1])$coords
links <- read.table("lectureData/08.missing/links.txt")
...
```{r, echo = FALSE, out.width="30%", fig.align='center'}
include_graphics("lectureData/08.missing/FishMinMissing.png")
```
...
```{r, echo = FALSE, out.width="30%", fig.align='center'}
include_graphics("lectureData/08.missing/FishMaxMissing.png")
```
...
```{r, echo = FALSE, out.width="70%", fig.align='center'}
include_graphics("LectureData/08.missing/TPSMissingConcept.png")  
```
...
include_graphics("LectureData/08.missing/MethodsCompare.png")  
```
