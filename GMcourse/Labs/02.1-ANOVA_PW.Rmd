...
Here are some examples for performing ANOVA-based models and pairwise comparisons in `geomorph` and `RRPP`. First, let's load some data: 
...
The pairwise test above is most appropriate.  It considers which means are different, after accounting for species and site effects. We could ignore that and use a different null model.  It is always wise to consider the most appropriate null model. 
Following is a poor choice, run just to illustrate how one can control the analysis.
